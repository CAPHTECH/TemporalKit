...
final class NestedDFSTests: XCTestCase {
    ...
    func testNestedDFS_StronglyConnectedComponents() throws {
        ...
        if let run = modifiedResult {
            ...
            XCTAssertTrue(prefixSet.intersection([1, 2, 3]).count > 0, "Prefix should pass through SCC1")
            XCTAssertTrue(prefixSet.intersection([4, 5]).count > 0, "Prefix should pass through SCC2")
        }
    }
} 
